...
class RegistrationFOSUser1Controller extends ContainerAware
{
    public function registerAction(Request $request)
    {
        ...
        if  ($form->isSubmitted()){
            $this->container->get('session')->getFlashBag()->set('error', 'Электронная почта подтверждена и используется');
        }
...
//        die();
        ...
        if ($redirectRoute = $this->container->getParameter('sonata.user.register.confirm.redirect_route')) {
            $response = new RedirectResponse($this->container->get('router')->generate('main_index', $this->container->getParameter('sonata.user.register.confirm.redirect_route_params')));
        } else {
